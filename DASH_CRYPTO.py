...
def get_global_market():
    ...
    for n, s in tickers.items():
        try:
            t = yf.Ticker(s)
            hist = t.history(period="1mo") # Besoin d'histo pour la volatilité
            fi = t.fast_info
            
            if fi.last_price and not hist.empty:
                var = ((fi.last_price - fi.previous_close)/fi.previous_close)*100
                volatility = hist['Close'].pct_change().std() * 100 # Volatilité sur 1 mois
...
if page == "Vue Globale 🌍":
    ...
    if not df.empty:
        ...
        with c_chart:
            # --- RETOUR DE LA MATRICE VOLATILITÉ vs PERF ---
            ...
            # Amélioration du design du chart
            fig.update_traces(
